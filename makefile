...
Sitcord.app/Contents/document.wflow: Sitcord.app/Contents/document.template.wflow
	(if [ -e $(shell echo "$(DISCORD_PATH)" | sed 's|\\|\\|g') ]; then \
		sed 's|echo|$(DISCORD_PATH) --remote-debugging-port=54321 \&gt; $(current_dir)bin/discord.log 2\&gt; $(current_dir)bin/discord.err \&amp;\|DISCORD_SERVER_NAME="$(SERVER)" DISCORD_DEBUG_PORT=54321 $(DISCORD_CHANNEL_NAME) PATH=$(PATH) $(current_dir)bin/sitcord \&gt; $(current_dir)bin/sitcord.log 2\&gt; $(current_dir)bin/sitcord.err \&amp;\||g' Sitcord.app/Contents/document.template.wflow | tr '|' '\n' > Sitcord.app/Contents/document.wflow; \
	fi)
